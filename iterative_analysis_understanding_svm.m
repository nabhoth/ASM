function [ ] = iterative_analysis_understanding_svm(flabels, att_data_mean1, model1, max_vals1, U1, B1, min_vals1, att_data_mean2, model2, max_vals2, U2, B2, min_vals2)
...
[C ia ib] = intersect([{i_ale(:).name}], [{i_cpmc(:).name}]); %to make sure names are same for the same images
iids = i_ale(ia)
...
for i = 1:numel(iids)
    try
        ...
        %% Boosting      
        %[idx] = predict(model1, cell2mat(evidence(1,1:k)));
        ...
        if s_size(1) > 2
            ...
            while strcmp(conditionEnd, 'false')
                ...
                for j=1:ar
                    ...
                    %% Boosting
                    %[algo] = predict(model2, cell2mat(evidence(1,1:k)));                  
