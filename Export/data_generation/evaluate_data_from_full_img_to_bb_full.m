function [results] = evaluate_data_from_full_img_to_bb_full()
% function [results] = evaluate_data_for_bn()
% evaluates the results of regional processing and termines which
% algorithms obtains the best result for a given region. Uses standard F
...
load([output_path '/from_whole_images_best_analysis/Features_Hypothesis_Regions_Mapping_BN_Test_New_extended.mat'], 'mapping');
...
for n=1:length(gtids)
    ...
    for j=1:numel(recs.objects)
        ...
        [h,w] = size(mapping);
        [x,y] = find(ismember(mapping(:,1),jgtids(1:4)) & ismember(mapping(:,2), jgtids(6:end)));   %%% remove j
        if (~isempty(x) && ~isempty(y))
            ...
            for k=1:numel(algo_paths)
                try
                    ...
                    [resfile,MAP] = imread([algo_paths{k} imname '.png']);                  
                    resfile = resfile(recs.objects(j).bbox(2):recs.objects(j).bbox(4), recs.objects(j).bbox(1):recs.objects(j).bbox(3),:);  %% =remove
                    imwrite(resfile, MAP, [algo_paths{k} jgtids '.png'], 'PNG');                 % remove   

                    m = ((k-1)*3)+3;
                    [results{all_count,m+1},results{all_count,m+2}, results{all_count,m+3}] = eval_symb_seg( resfile, mask, gt.indx);  %% modify without mask                  
...
save_path = [output_path '/from_whole_images_best_analysis/nobins__reduced_false_evalresult_full_piece_5algos.mat']; %nobins__reduced_false_evalresult_full_piece_4algos
