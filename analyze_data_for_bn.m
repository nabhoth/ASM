...
function [suitability, im_accumulators, rows,  ent_results,  per_results, classes_cat, c_mu, a_mu, a_cov, c_cov,results, full_results, ful_results,norm_results,norm_results_s, max_vals] = analyze_data_for_bn(bins, removal, threshold, minmax, reduce)
%function [suitability, f_labels, im_accumulators, only_data, all_data, obj_data, all_images, only_numeric,plots, ent_results,  per_results, classes_cat, c_mu, a_mu, a_cov, c_cov ] = analyze_data_for_bn(bins, removal, threshold)
% function [ ] = analyze_data_for_bn()
% analyzes teh data by looking at individual features and attributes
% parameters:
%               bins and removal - determines the type of input results to be used
%               threshold - selects only data abovecertain threshold of result
%               minmax - if set to 1 it will only considers the MinMax data
%               - bvery good for one algorithm and very bad for the other
%               one
...
algo_path = {'./data/Results/ale_val_cls/', './data/Results/cpmc_val_cls/'};
...
%     
%     
%     [~,p] = find(strcmp(ful_results(1,:), 'brightness'));
...
%     end
    ...
    [~,p] = find(strcmp(non_clustered(1,:), 'brightness'));
    ...
    %full results binned
    [~,p] = find(strcmp(results(1,:), 'brightness'));
    ...
    if reduce > 0
        ...
        results(2:end,p:end) = num2cell(newdata(:,p:end));
    end
